...
int main(int argc, char ** argv) {
  ...
  if (process_rank == 0) {
    ...
    for (int i = 0; i < num_slices; i++) {
      int start = (i * list_size) / num_slices;
      int end = ((i + 1) * list_size) / num_slices;
      // since end is not a closed interval I omited + 1
      int worker_list_length = end - start;

      MPI_Send(&worker_list_length, 1, MPI_INT, i + 1, 0, MPI_COMM_WORLD);

      MPI_Send(&(list[start]), worker_list_length, MPI_FLOAT, i + 1, 0, MPI_COMM_WORLD);
    }
    ...
    MPI_Recv(&list_length, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

    list = (float *) malloc (list_length * sizeof(float));

    MPI_Recv(list, list_length, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
